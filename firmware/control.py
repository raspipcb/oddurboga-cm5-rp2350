...
class Controller:
    ...
    def recover(self):
        ...
        self.flow_wanted = False
        self.actuators.close_inlet(allow=True)
    ...
    def status_fields(self):
        # FLOW is logical: inlet path active (hot or cold valve open, or fill wanted).
        flowing = self.flow_wanted and self.phase in (
            self.PHASE_PRECOOL, self.PHASE_FLOW, self.PHASE_REGULATE,
        )
        flow = "ON" if flowing or self.actuators.inlet_open() else "OFF"
    ...
    def tick(self):
        ...
        if actions["kill_flow"]:
            self.flow_wanted = False
            self.actuators.close_inlet(allow=False)
            self.fill.stop(record=False)
    ...
    def _tick_precool(self, allow):
        """Open cold valve only for MIX_PRECOOL_S before allowing hot."""
        self.busy = True
        self.mix.phase = "PRECOOL"
    ...
    def _tick_open_flow(self, allow):
        # Inlet path is already the hot/cold valves; begin regulate + fill timer.
    ...
    def _tick_heat_cable(self, allow):
        ...
        if mode == "ON":
            on = True
        elif mode == "AUTO" and self.store.frost_active:
            on = True
